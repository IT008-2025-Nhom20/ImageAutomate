...
*   **`IncludeSchemaReference`**: If true, includes a `$schema` reference in the JSON output.

## Methods

### Hit Testing
*   **`HitTestNode(double x, double y)`**: Returns the top-most `IBlock` at the specified world coordinates.
...
*   **Embedded Layout**: During serialization, layout information (X, Y, Width, Height) is embedded into the block objects.