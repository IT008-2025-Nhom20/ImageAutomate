...
## Description
The Resize Block resizes images to new dimensions. It supports various resize modes to handle aspect ratio preservation, cropping, and padding.
...
### Execution
The block applies `Image.Mutate(x => x.Resize(options))` using `SixLabors.ImageSharp`.