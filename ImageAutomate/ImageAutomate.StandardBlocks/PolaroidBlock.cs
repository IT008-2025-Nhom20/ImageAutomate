...
public class PolaroidBlock : IBlock
{
    #region Fields

    private readonly IReadOnlyList<Socket> _inputs = [new("Polaroid.In", "Image.In")];
    private readonly IReadOnlyList<Socket> _outputs = [new("Polaroid.Out", "Image.Out")];
    ...
    private int _width;
    private int _height;
    private string _title = "Polaroid";

    #endregion

    public PolaroidBlock()
        : this(200, 100) 
    {
    }

    public PolaroidBlock(int width, int height)
    {
        _width = width;
        _height = height;
    }
        ...
        IDictionary<string, IReadOnlyList<IBasicWorkItem>> inputs, CancellationToken cancellationToken)
    {
        ...
        foreach (var sourceItem in inItems.OfType<WorkItem>())
        {
            ...
            // Apply the Polaroid effect (adds white border and rotation)
            sourceItem.Image.Mutate(x => x.Polaroid());
