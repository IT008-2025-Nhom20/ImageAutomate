...
public class LomographBlock : IBlock
{
    #region Fields

    private readonly IReadOnlyList<Socket> _inputs = [new("Lomograph.In", "Image.In")];
    private readonly IReadOnlyList<Socket> _outputs = [new("Lomograph.Out", "Image.Out")];
    ...
    private int _width;
    private int _height;
    private string _title = "Lomograph";

    #endregion

    public LomographBlock()
        : this(200, 100) // Compact size
    {
    }

    public LomographBlock(int width, int height)
    {
        _width = width;
        _height = height;
    }
        ...
        IDictionary<string, IReadOnlyList<IBasicWorkItem>> inputs, CancellationToken cancellationToken)
    {
        ...
        foreach (var sourceItem in inItems.OfType<WorkItem>())
        {
            ...
            // Apply Lomograph effect
            sourceItem.Image.Mutate(x => x.Lomograph());
